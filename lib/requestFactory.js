...
var createRequest = function(organizationEndpoint, tokens, entityLogicalName, attributes) {
    var attributesArray = attributes || [];
    var attrStrings = '';
    for (var i = 0; i < attributesArray.length; i++) {
        var type = attributesArray[i].type ? attributesArray[i].type : 'string';

        attrStrings += '<a:KeyValuePairOfstringanyType>' +
            '<b:key>' + attributesArray[i].logicalName + '</b:key>' +
            '<b:value i:type="c:' + type + '" xmlns:c="http://www.w3.org/2001/XMLSchema">' + attributesArray[i].value + '</b:value></a:KeyValuePairOfstringanyType>';
    }

  var header = createHeader(organizationEndpoint, tokens, 'Create');
  var body = templates.createRequestBodyTemplate
    .replace('{logicalname}', entityLogicalName)
    .replace('{attributes}', attrStrings);
